        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-lg font-bold text-primary-900">Visitors</h1>
        ...
        <div className="card-corporate">
          {loading ? (
                  ...
                  <tbody>
                    {visitors.data.map((visitor) => (
                      <tr key={visitor.id}>
                        <td>
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              {visitor.phone && (
                                <p className="text-xs text-neutral-500">{visitor.phone}</p>
                              )}
                            </div>
                          </div>
                        </td>
                        ...
                        <td>
                          <span className="badge badge-primary">{visitor._count.visits}</span>
                        </td>
                        <td>{new Date(visitor.createdAt).toLocaleDateString()}</td>
                        <td className="text-right">
                          <button className="text-accent-600 hover:text-accent-700 font-medium text-sm mr-3">
                            View
                          </button>
                          <button
              ...
              <div className="card-corporate-footer flex items-center justify-between">
                <p className="text-sm text-neutral-500">
                  ...
                  <span className="font-medium text-primary-900">
                    {(visitors.page - 1) * visitors.pageSize + 1}
                  </span>{" "}
                  to{" "}
                  <span className="font-medium text-primary-900">
                    {Math.min(visitors.page * visitors.pageSize, visitors.total)}
                  </span>{" "}
                  of <span className="font-medium text-primary-900">{visitors.total}</span> visitors
                </p>
            ...
            <div className="p-12 text-center">
              ...
              <p className="text-sm text-neutral-400 mt-1">Try adjusting your search or add a new visitor</p>
            </div>
          )}
        </div>
      </main>
