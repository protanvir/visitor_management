      ...
      <main className="max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            ...
            <div className="stat-card text-center">
              <p className="stat-value text-success-600">
                {Object.entries(stats.actionCounts)
                  .filter(([k]) => k.includes("created") || k.includes("completed"))
                  .reduce((sum, [, v]) => sum + v, 0)}
              </p>
              <p className="stat-label">Created</p>
            </div>
            <div className="stat-card text-center">
              <p className="stat-value text-accent-600">
                {Object.entries(stats.actionCounts)
                  .filter(([k]) => k.includes("checked_in") || k.includes("signed"))
                  .reduce((sum, [, v]) => sum + v, 0)}
              </p>
            ...
            <div className="stat-card text-center">
              <p className="stat-value text-danger-600">
                {Object.entries(stats.actionCounts)
                  .filter(([k]) => k.includes("deleted") || k.includes("rejected"))
                  .reduce((sum, [, v]) => sum + v, 0)}
              </p>
        ...
        <div className="flex flex-wrap gap-4 mb-6">
          ...
          >
            ...
            <option value="safety.completed">Safety Completed</option>
            <option value="badge.returned">Badge Returned</option>
          </select>
        ...
        <div className="card-corporate">
          {loading ? (
                  ...
                  <tbody>
                    {logs.map((log) => (
                      <tr key={log.id}>
                        ...
                        <td>
                          <div>
                            <p className="text-sm font-medium text-primary-900">{log.entityType}</p>
                            <p className="text-xs text-neutral-500">{log.entityId.slice(0, 8)}...</p>
                          </div>
                        </td>
            ...
            <div className="p-12 text-center">
              ...
              <p className="text-sm text-neutral-400 mt-1">Activity will appear here as users interact with the system</p>
            </div>
          )}
        </div>
      </main>
