...
export default function SitesManagementPage() {
  ...
  const fetchSites = async () => { setLoading(true); try { const token = localStorage.getItem("token"); const params = new URLSearchParams(); if (search) params.append("search", search); const res = await fetch(`http://localhost:3001/api/sites?${params}`, { headers: { Authorization: `Bearer ${token}` } }); const r = await res.json(); if (r.success) setSites(r.data || []); else setError(r.error); } catch (e) { setError("Failed"); } finally { setLoading(false); } };
  const handleCreate = async () => { setFormError(""); setFormLoading(true); try { if (!user?.organizationId) { setFormError("No org"); setFormLoading(false); return; } const token = localStorage.getItem("token"); const res = await fetch("http://localhost:3001/api/sites", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ ...formData, organizationId: user.organizationId }) }); const r = await res.json(); if (r.success) { setShowModal(false); setFormData({ name: "", address: "", timezone: "UTC" }); fetchSites(); } else setFormError(r.error); } catch (e) { setFormError("Failed"); } finally { setFormLoading(false); } };
  const handleUpdate = async () => { if (!editing) return; setFormError(""); setFormLoading(true); try { const token = localStorage.getItem("token"); const res = await fetch(`http://localhost:3001/api/sites/${editing.id}`, { method: "PUT", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify(formData) }); const r = await res.json(); if (r.success) { setEditing(null); setShowModal(false); setFormData({ name: "", address: "", timezone: "UTC" }); fetchSites(); } else setFormError(r.error); } catch (e) { setFormError("Failed"); } finally { setFormLoading(false); } };
  const handleDelete = async (id: string) => { if (!confirm("Delete?")) return; try { const token = localStorage.getItem("token"); const res = await fetch(`http://localhost:3001/api/sites/${id}`, { method: "DELETE", headers: { Authorization: `Bearer ${token}` } }); const r = await res.json(); if (r.success) fetchSites(); else alert(r.error); } catch (e) { alert("Failed"); } };
